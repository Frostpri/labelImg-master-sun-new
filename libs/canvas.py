...
class Canvas(QWidget):
    ...
    def copyShape(self):
        ...
        if shape is None:
            return
        # 获取标注形状的图像和标签
        image = self.image.copy(shape.points[0].x(), shape.points[0].y(), shape.points[2].x() - shape.points[0].x(),
                                shape.points[2].y() - shape.points[0].y())
        label = shape.label
        # 复制标注形状
        newShape = shape.copy()
        newShape.label = label
        newShape.line_color = QColor(0, 255, 0, 128)
        newShape.fill_color = QColor(0, 255, 0, 128)
        # 将复制的标注形状添加到画布中
        ...
        # 在 copy 程序中，导入 subprocess 模块
        import subprocess

        # 在 copy 程序中，修改发送请求的部分，改为调用 run 程序
        # 假设 run 程序的文件名是 run.py，您可以根据您的实际情况修改
        # 构造请求的数据
        data = {
        ...
        # 调用 run 程序，将数据作为标准输入，获取标准输出
        process = subprocess.run(["python", "run.py"], input=data_json, text=True, capture_output=True)
        # 处理输出
        if process.returncode == 0:
            # 调用成功
            print("Copy and run successfully!")
            # 从标准输出中读取 json 数据
            output_json = process.stdout
            # 解析 json 数据
        ...
        # 处理响应
            ...
            # 请求成功
            #print("Copy and send successfully!")
        #else:
            # 请求失败
            #print("Copy and send failed!")
        # 删除临时文件
        #image_file.close()
